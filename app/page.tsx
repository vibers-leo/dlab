  ...
  return (
    ...
    >
        ...
        {SECTIONS.map((_, i) => (
          <button
            ...
            className={`rounded-full transition-all duration-300 ${
              current === i ? 'w-2.5 h-2.5 bg-black scale-125' : 'w-2 h-2 bg-gray-400'
            }`}
      ...
      <section
        ...
        style={{ scrollSnapAlign: 'start', backgroundColor: '#FAFAFA' }}
      >
        {/* 로고 */}
        <div className="absolute top-8 left-10 text-black font-bold text-lg tracking-tight">
          D.Lab
        </div>

        {/* 중앙 콘텐츠 */}
        <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
          <span className="text-gray-400 text-sm tracking-widest mb-6">소셜벤처 · Social Venture</span>
          <h1 className="text-7xl sm:text-8xl font-extrabold text-black leading-tight mb-2">
            세상을 이롭게,
          </h1>
          <h2 className="text-7xl sm:text-8xl font-extrabold text-gray-400 leading-tight mb-8">
            디랩이 만듭니다.
          </h2>
          ...
          <div className="flex gap-4">
            <a
              ...
              className="px-8 py-3.5 bg-black text-white font-semibold rounded-none text-sm tracking-wide hover:bg-gray-800 transition-colors"
            >
              프로젝트 상담 →
            </a>
            <button
              onClick={() => scrollTo(3)}
              className="px-8 py-3.5 border border-black text-black font-semibold text-sm tracking-wide hover:bg-black hover:text-white transition-colors"
            >
              작업물 보기
            </button>
          </div>
        ...
        {/* 스크롤 힌트 */}
        <div className="absolute bottom-8 right-10 text-gray-400 text-sm tracking-widest">
          ↓ scroll
        </div>
      </section>
      ...
      >
        ...
        <div className="relative z-10 max-w-5xl w-full">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-8">
            {[
              { title: '이벤트·축제', desc: '지자체·기업 행사의 공식 디지털 창구를 제작합니다' },
              { title: '입찰·제안서', desc: '경쟁 입찰에서 차별화되는 웹 제안서를 만듭니다' },
              { title: '전시·갤러리', desc: '오프라인 전시를 온라인으로 확장합니다' },
              { title: '앱·웹 개발', desc: '기획부터 출시까지 한 팀이 책임집니다' },
            ].map((item) => (
              <div key={item.title}>
          ...
          <div className="flex flex-col gap-10">
            {[
              ...
              <div key={item.num} className="flex items-baseline gap-4">
                <span className="text-6xl sm:text-7xl font-extrabold text-black">{item.num}</span>
                <div>
      ...
      >
        <div className="max-w-5xl w-full mx-auto grid grid-cols-1 sm:grid-cols-2 gap-16 items-start">
          ...
          <div className="flex flex-col divide-y divide-gray-100">
            {[
              { num: '01', name: '이벤트·축제 홈페이지', desc: '참가 신청, 일정, 지도까지 — 2~4주 내 런칭' },
              { num: '02', name: '입찰 제안서 웹화', desc: '열람 통계, 모바일 최적화로 심사위원 임팩트 극대화' },
              { num: '03', name: '전시·갤러리 디지털화', desc: '작품 아카이브, 도슨트 연동, 디지털 팸플릿' },
              { num: '04', name: '앱·웹 서비스 개발', desc: 'iOS/Android/웹 풀스택, 스토어 출시까지 책임' },
            ].map((item) => (
              <div key={item.num} className="py-6 flex gap-6 items-start group cursor-default">
                <span className="text-gray-300 font-bold text-sm mt-1">{item.num}</span>
                <div>
                  <h3 className="font-bold text-black text-lg group-hover:text-gray-600 transition-colors">{item.name}</h3>
          ...
          <div className="flex flex-col gap-6">
            {[
              ...
              <div key={item.step} className="flex items-center gap-5">
                <span className="text-indigo-400 font-bold text-sm w-6 shrink-0">{item.step}</span>
        ...
        <div className="max-w-4xl w-full mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="border border-gray-200 p-10">
              ...
              <button
                onClick={() => scrollTo(6)}
                className="mt-8 w-full py-3 border border-black text-black font-semibold text-sm hover:bg-black hover:text-white transition-colors"
              >
                문의하기
              </button>
            </div>
            ...
            <div className="bg-black p-10">
              <p className="text-gray-400 text-sm tracking-widest mb-4">연간 파트너</p>
              <p className="text-5xl font-extrabold text-white mb-1">월 200만원~</p>
              <p className="text-gray-500 text-sm mb-8">12개월 단위</p>
              ...
              <button
                onClick={() => scrollTo(6)}
                className="mt-8 w-full py-3 bg-white text-black font-semibold text-sm hover:bg-gray-100 transition-colors"
              >
                파트너 계약 시작하기
              </button>
            </div>
      ...
      >
        <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
          ...
          <p className="text-gray-500 text-lg mb-10">hello@designdlab.co.kr</p>
          <div className="flex flex-col sm:flex-row gap-4">
            <a
              href="https://open.kakao.com/o/dlab"
              className="px-8 py-4 font-bold text-black text-sm tracking-wide"
        ...
        <footer className="border-t border-white/10 py-6 px-10 flex flex-col sm:flex-row justify-between items-center gap-2">
          <span className="text-gray-600 text-sm">D.Lab · 계발자들 협력사 · vibers.co.kr</span>
          <span className="text-gray-700 text-sm">© {new Date().getFullYear()} 디랩. All rights reserved.</span>
        </footer>
      </section>
    </div>
  );
